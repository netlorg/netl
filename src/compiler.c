...
/*==============================================================================
| this is the netl compiler.  actually, it's just the code generation phase
| of the compiler, the compiler consists of this, and the config parser.
| unfortunately, i wrote this module in my "i hate compilers which aren't gcc"
| frame of mind and have several multi line string constants which will cause
| most compilers to complain (unreasonably, i think).  this is something
| which we will need to fix at a later date.
|
| one workaround i am considering is seperating CC from LD.  the problem right
| now is with AIX (incidentally, it's friday and everyone is pissing off early
| ; three day weekend), which requires gcc to compile, but xlc to link with
| -lpcap.  if we set CC=gcc and LD=xlc, we may be able to get around this, but
| that is a bad workaround.  hrm.  suggestions?  the other option is to just
| convert the multi-line string constants into a series of concatinated string
| constatnts, like i did in options.c 26 may 2000...
|=============================================================================*/
...
static char *int2prot[4] = { "TCP", "UDP", "ICMP", "IGNP" };
static char *int2act[7] = { "NONE", "LOG", "DUMP", "IGNORE", "DL", "OTHER", "NULL" };
static char *prot2logtype[6] = { "ip_with_port", "ip_with_port", 
			"ip_no_port", "ip_no_port",
			"unknown", "ip_unknown" };

void
netl_generate_c(FILE *fp)
{
	int i, n, prot = -1;
...
dump(u8 *dg, char *logname, size_t len)
{
	...
	if(logname[0] != 0) {
#ifdef NO_SNPRINTF
		sprintf(fn, \"/tmp/netl/%s-%d-%d-%d.dg\", logname, getpid(), (unsigned) time(NULL), sequence++);
#else
