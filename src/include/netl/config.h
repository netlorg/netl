...
#ifndef NETL_CONFIG_H
#define NETL_CONFIG_H
...
struct configitem {
  ...
  char		*format;
  /*
  void		(*actionf)(u8 *, struct configitem *, size_t, int);
  int		*action_done;
   */
  int		num_actionmod;
...
#ifdef NO_SYSLOGD
void netl_config_readfile(char *confname);
#endif
#ifndef NO_SYSLOGD
void netl_config_readfile(char *confname, int nbg);
#endif
void netl_config_pre(void);
void netl_config_post(void);
void netl_config_clear(void);
void netl_config_set(struct configlist *);
void netl_config_parseline(char *s);
