/*==============================================================================
| tcp
|   parse a IP/TCP datagram. (IP 4/6)
|
...
construct(void)
{
	ip4 = lookup_filter("tcp4", PROT_TCP4);
	ip6 = lookup_filter("tcp6", PROT_TCP6);
}
...
void
filt_tcp_register_symbols(void)
{
	register_symbol("filt/tcp.so", "req", &req);
	register_symbol("filt/tcp.so", "check", check);
	register_symbol("filt/tcp.so", "construct", construct);
}
#endif
