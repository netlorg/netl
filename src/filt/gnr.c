...
check(u8 *dg, size_t len)
{
	...
	union { icmphdr i; tcphdr t; udphdr u; } *h = 
			(void *) &dg[(IPIHL(ip->ihl_version) << 2) + 14];
...
void
filt_gnr_register_symbols(void)
{
	register_symbol("filt/gnr.so", "req", &req);
	register_symbol("filt/gnr.so", "check", check);
}
#endif
