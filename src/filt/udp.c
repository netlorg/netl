/*==============================================================================
| udp
|   parse a IP/UDP datagram. (IP 4/6)
|
...
construct(void)
{
	ip4 = lookup_filter("udp4", PROT_UDP4);
	ip6 = lookup_filter("udp6", PROT_UDP6);
}
...
void
filt_udp_register_symbols(void)
{
	register_symbol("filt/udp.so", "req", &req);
	register_symbol("filt/udp.so", "check", check);
	register_symbol("filt/udp.so", "construct", construct);
}
#endif
