...
#define line_number netl_guts_yy_line_number
#define yyin netl_guts_yy_in

int netl_guts_yy_parse(void);
static int netl_guts_yy_error(char *s);
int netl_guts_yy_lex(void);
static int modifyip(u32 *num, char *name);
static void detectf(void);
static void additem(struct configlist *l, struct configitem *c);

extern signed int netl_config_listenport;
...
#ifdef NETL_CONFIG_Y_C
						netl_config_listenport = htons($<x.i>2);
...
*/
	...
	|	RULE_LISTEN NL		{
						...
						netl_config_listenport = htons(47);
						lookup_filter("dcp", PROT_USER);
#endif
#ifdef NETL_CONFIG_Y_PERL
/*
...
fld :		op_not FLD_NAME str
			{
				...
				if(ci.logname != NULL)
					free(ci.logname);
				len = strlen($<x.s>3);
				ci.logname = malloc(len+1);
				strcpy(ci.logname, $<x.s>3);
				if($<x.i>1) 
...
signed int netl_config_listenport = -1;	/* oh my gosh, i'm wasting 15 BITS!!!! */
...
void
netl_config_set(struct configlist *l)
{
...
void
netl_config_pre(void)
{
}

void
netl_config_post(void)
{
}

/*==============================================================================
| netl_config_readfile(char *prog)
|
...
#ifdef NO_SYSLOGD
void
netl_config_readfile(char *confname)
#endif
#ifndef NO_SYSLOGD
void
netl_config_readfile(char *confname, int nbg)
#endif
{
	extern FILE *yyin;
