...
#ifdef NO_SNPRINTF
	sprintf(buffer, "%s/filt/%s.so", netl_config_so_path, name);
#else
	snprintf(buffer, 1024, "%s/filt/%s.so", netl_config_so_path, name);
#endif
