...
grab(int *len)
{
	...
	*len = recvfrom(sock, buf, 4096, 0, (struct sockaddr *) &name, &fromlen);
	return buf;
}

#if BOOL_DYNAMIC_MODULES == 0
void    
in_linux_ether_register_symbols(void)
{
	register_symbol("in/linux-ether.so", "prepare", prepare);
	register_symbol("in/linux-ether.so", "grab", grab);
}       
#endif  
